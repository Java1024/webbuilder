package org.webbuilder.office.word.support.template;
...
public class DOCXTemplateReader {
    ...
    public static final List<Map<String, Object>> text2object(String txt, String template) {
        ...
        while (matcher.find()) {
            ...
            if (express_end + 10 < template.length()) {
                endOf = template.substring(express_end, express_end + 10);
            }
            if (endOf.contains("${"))
                endOf = " ";
            if (index > 10) {
                startOf = template.substring(index - 10, index);
            }
